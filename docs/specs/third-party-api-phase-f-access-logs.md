...
| T9  | **Follow-up (2026-09-01, see §B3/§B4):** fix `createEntity`'s `fireEntityAuditHook` actorId bug; harden the admin route against legacy non-uuid `actorId` rows; extend audit-writing to all 5 third-party read endpoints (ticket detail, ticket list, workflow list, workflow-fields describe, attachment download) with 6 new `AuditAction` values (migration 0088) and a `classifyRequestKind` read/write classifier; admin route gains a `type=read\|write` filter and drops its hardcoded `resourceType: "ticket"`; admin-ui switches from cursor "Load more" to real Previous/Next pagination at 20/page and adds the Type filter     | 3     | done   | T2, T3         |
