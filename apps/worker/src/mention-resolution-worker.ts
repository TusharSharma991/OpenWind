...
export const mentionResolutionWorker = new Worker<MentionResolutionJob>(
  ...
  async (job) => {
    ...
    if (outcome === 3) {
      ...
      // Policy (2026-09-08, revised from an earlier synchronous 422-on-
      ...
      // and a known identifier with no tenant "user" role) with the same
      // generic wording -- distinguishing them in the message would
