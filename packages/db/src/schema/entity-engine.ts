...
export const entityInstances = pgTable(
  ...
  (t) => ({
    tenantTypeIdx: index('entity_instances_tenant_type_idx').on(t.tenantId, t.entityTypeId),
    tenantStateIdx: index('entity_instances_tenant_state_idx').on(t.tenantId, t.currentState),
  })
);
...
export const entityRelations = pgTable(
  'entity_relations',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    tenantId: uuid('tenant_id').notNull(),
    fromInstanceId: uuid('from_instance_id').notNull().references(() => entityInstances.id),
    toInstanceId: uuid('to_instance_id').notNull().references(() => entityInstances.id),
    relationType: text('relation_type').notNull(),
    createdAt: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  },
  (t) => ({
    fromIdx: index('entity_relations_from_idx').on(t.tenantId, t.fromInstanceId),
