...
| ID        | Question                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     | Phase      |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- |
| **WE-01** | Still open, and reframed (confirmed 2026-08-24): the `workflows` table has no version column whatsoever — only `isActive`. Issue #62 (closed 2026-08-02) found no workflow-versioning concept exists in the shipped architecture at all, so this question's original framing ("what happens during retirement") is moot — there's no version to retire. The real open question is whether workflow versioning should be built at all, not how to handle its retirement.                                                                                                                                                                                                                                                                                                                                                                                                                      | Phase 2    |
| **WE-02** | ~~Can a workflow transition be undone?~~ **Resolved ([issue #64](https://github.com/TinyPhi/OpenWind/issues/64)):** Transitions are **irreversible by design**. `workflow_events` is an append-only audit log — no UPDATE or DELETE policy exists, and none will be added. If a workflow definition requires a rollback path, the workflow author must define an explicit reverse transition (e.g. `approved → rejected`). There is no `undo_transition` API endpoint and none is planned. This makes the event log a reliable audit trail and eliminates the consistency problems that arise from retroactive state mutation.                                                                                                                                                                                                                                                               | Phase 2 ✅ |
| **WE-03** | ~~How are SLA breaches handled when BullMQ is down for an extended period? Are missed SLA firings backfilled on recovery or permanently lost? Define the recovery behaviour explicitly.~~ **Resolved in [issue #63](https://github.com/TinyPhi/OpenWind/issues/63):** On worker restart the SLA scheduler re-polls undelivered `workflow.sla_scheduled` outbox events. Events whose `fireAt` is in the past but within a 48-hour window are re-enqueued with `delay=0` and fire immediately (BullMQ deduplication via deterministic job IDs prevents double-firing). Events older than 48 hours are written to `dead_letter_events` for operator inspection rather than being silently lost or blindly re-fired. The sla-breacher logs a warning with `latencyMs` when a job fires more than 15 minutes past its scheduled `fireAt`, surfacing recoveries without blocking the breach event. | Phase 1 ✅ |
| **WE-04** | Still open (confirmed 2026-08-24): `condition-evaluator.ts` implements a fixed operator set (`eq`, `neq`, `gt`, `gte`, ...) with no extensibility hook, changelog, or RFC process documented anywhere. No owner or process is named in any doc or issue. This is a governance question code cannot answer on its own.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | Phase 2    |
| **WE-05** | ~~Is `getAvailableTransitions` the single authoritative source for transition availability, or can the UI also apply additional filters? If both, what happens when they disagree?~~ **Resolved (2026-08-24):** Yes. `apps/api/src/routes/entities/list-transitions.ts` calls `getAvailableTransitions` (`packages/workflow-engine`) directly with no additional business filtering — the only thing layered on top is an access-control gate (`hasEntityAccess`) that determines whether the caller may see the record at all (404, not empty transitions, if not). No UI-side transition filtering exists or is architecturally sanctioned; a UI hiding a transition cosmetically doesn't change what the API enforces.                                                                                                                                                                    | Phase 1 ✅ |
...
| **WE-07** | Still open (confirmed 2026-08-24): no cycle/DFS/topological-sort logic exists anywhere in `packages/workflow-engine/src`, including `createWorkflow`'s transition-creation path. Nothing rejects a cyclic transition graph with no reachable terminal state at config time. Not currently tracked by its own issue.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | Phase 3    |
