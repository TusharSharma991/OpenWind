...
- Every third-party API request — read or write — writes an `admin_audit_log` entry: allowed on
  success, and (where a real ACL/access-list check exists, not just a genuinely-missing-resource 404) on denial too. A read-only key that only ever fetches ticket/workflow data must leave the
  same trail a mutating key does, per §B4. Read vs write is a derived property of the action name
  (`classifyRequestKind`, mirroring `classifyOutcome`'s own centralized-mapping pattern) — never a
  second stored column.
- Any code path that writes `admin_audit_log` for a third-party actor — not just direct
  `writeAuditEntry` call sites, but also indirect ones like `createEntity`'s
  `fireEntityAuditHook` — must set `actorId` to the API key's own id, never `createdBy`/
  `actingPersonId`, per §B3. When adding a new indirect write path, grep for how it resolves its
  own `actorId`/`actorType` inputs specifically, not just whether it calls something
  audit-shaped.
- The admin access-logs route must never let one malformed/legacy `actor_id` (a non-uuid value
  from a since-fixed bug) 500 the entire response — filter to well-formed values before using a
  row's `actor_id` in a typed (e.g. uuid) column comparison, and resolve the rest to an
  "unknown"/null display value instead, per §B3.
...
| T9  | **Follow-up (2026-09-01, see §B3/§B4):** fix `createEntity`'s `fireEntityAuditHook` actorId bug; harden the admin route against legacy non-uuid `actorId` rows; extend audit-writing to the third-party read endpoints present on this branch (ticket detail, workflow list, workflow-fields describe, attachment download — `GET .../tickets` list isn't in this branch's router yet, see the endpoint-index note) with 6 new `AuditAction` values (migration 0089, one — `ticket.listed` — reserved unused until that route lands) and a `classifyRequestKind` read/write classifier; admin route gains a `type=read\|write` filter and drops its hardcoded `resourceType: "ticket"`; admin-ui switches from cursor "Load more" to real Previous/Next pagination at 20/page and adds the Type filter | 3     | done   | T2, T3         |
...
| B3  | `GET /admin/third-party-access-logs` 500'd in production (2026-09-01): `createEntity`'s `fireEntityAuditHook` call ignored its own `input.actorId` and used `input.createdBy` instead, so ticket-CREATE rows (unlike comment/child/attachment/transition) still wrote `actorId: actingPersonId` even after B2's fix landed on those other 4 routes. Postgres then rejected the whole `api_keys.id IN (...)` lookup when a non-uuid person id reached it. | B2's fix only touched the 4 routes that call `writeAuditEntry` directly; ticket creation goes through `createEntity` → `fireEntityAuditHook`, a 5th write path B2's grep-for-writeAuditEntry sweep didn't cover because it doesn't call `writeAuditEntry` by name.                                                             | Yes — extends B2's invariant: any code path that ultimately calls `fireEntityAuditHook` (not just direct `writeAuditEntry` call sites) must be checked for the same actorId-vs-createdBy confusion. Also hardened the admin route itself to skip non-uuid legacy `actorId` values when resolving `applicationName`, instead of letting one bad row 500 the whole page — defense-in-depth against the _next_ such bug, not just this one. |
| B4  | The read-only endpoints (`GET /tickets/:id`, `GET .../tickets`, `GET /workflows`, `GET .../fields`, `GET /attachments/:id/download`) wrote **zero** `admin_audit_log` rows, allowed or denied — discovered when an admin asked "someone accessed our data and we aren't logging it — is that a security issue?" after noticing a read via the test app produced no new log row.                                                                          | R1's original scope ("every third-party request/attempt") was interpreted at implementation time as "every write" (matching Phase B's own R7 wording, which only ever discussed writes) — reads were never in scope for any Phase B–E route's audit-write pattern, so there was nothing for Phase F to retrofit onto for them. | Yes — promoted to §V below: every third-party route, read or write, must write an `admin_audit_log` entry on both its allowed and (where a real ACL check exists) denied outcome, never only on genuinely-missing-resource 404s.                                                                                                                                                                                                         |
