<!--
This is NOT the ADR — it's a proposed update for @ab to manually diff against and apply to
docs/decisions/ADR-015-observability-compliance.md. Claude Code cannot write to that path
directly (protected — ADRs are human-authored/edited per CLAUDE.md).

Source: PrabhuVijit's PR #501 review findings F-01 and F-03.

Changes from the current committed ADR-015, both in the header block only — body unchanged:
  1. F-01 (Medium): Status: Proposed. → Status: Accepted.
     (PR title says "accept ADR-015" — the field should match; if the real intent is still
     "propose, not yet accepted," don't apply this one and retitle the PR instead.)
  2. F-03 (Low): Deciders: Engineering Lead. → Deciders: Abhinav Mishra.
     (matches ADR-001–014's convention of naming a real person, not a role)

One thing to consider while you're in there: "Implementation next steps" item 1 below still
reads "@ab finalizes this draft, then moves it into docs/decisions/... with Status: Accepted" —
now stale once Status is actually Accepted (that step already happened). Not from the review,
just flagging it since you're editing the header anyway — your call whether to touch it now or
leave it as historical narrative.

Everything below this comment is the full ADR text with only the two header lines changed —
copy the whole thing over docs/decisions/ADR-015-observability-compliance.md, or just apply the
two line-diffs by hand.
-->
