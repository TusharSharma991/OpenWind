...
-- Get the entity type id we just created (use a CTE for subsequent inserts)
DO $$
DECLARE
  et_id UUID;
  wf_id UUID;
...
BEGIN
  ...
  WHERE tenant_id = '{TENANT_ID}' AND module_id = '{MODULE_ID}' AND name = 'Support Ticket'
  LIMIT 1;
  ...
  VALUES
    ...
    (gen_random_uuid(), wf_id, 'waiting_for_customer', 'in_progress',          'I''ve Responded',  '["admin","agent","user"]',false, '[]'),
